...
To install the railo_ctl service, you would just use:
...
the railo_ctl script. You can do that with:
    ...
    # $0 --install --path /path/to/railo_ctl
...
myMode=
myPath="../railo_ctl" # default path to railo_ctl script
...
function autodetectPath {
	...
	# "../railo_ctl" failed. Try the full default location instead:
	local defaultlocation="/opt/railo/railo_ctl";

	# now test it
	if [[ ! -f ${defaultLocation} ]] || [[ ! -x ${defaultLocation} ]]; then
		echo "[FAILED]";
		echo "* [ERROR]: NOT found in /opt/railo/railo_ctl...";
	else
		# looks good, set the variable
		myPath="/opt/railo/railo_ctl";
		local myFileFound=1;
		echo "[SUCCESS]";
	fi
	
	if [[ $myFileFound -eq 0 ]]; then
		echo "* [FATAL]: No railo_ctl file found and cannot autodetect!";
		echo "* [INFO]: Ensure the railo_ctl file is present and executable.";
		exit 1;
	fi
}
...
function install_railoCTL {
	...
	if [[ $myLinuxVersion == *RedHat*  ]]; then
		...
		# copy railo_ctl to /etc/init.d/
                cp $myPath /etc/init.d/railo_ctl
...
function remove_railoCTL {
        ...
        if [[ $myLinuxVersion == *RedHat*  ]]; then
                ...
                # install railo_ctl service
                update-rc.d -f railo_ctl remove
        fi
}
